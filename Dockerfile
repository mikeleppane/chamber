...
RUN case "$TARGETPLATFORM" in \
    "linux/amd64") \
        cp /usr/src/chamber/target/x86_64-unknown-linux-musl/release/chamber /usr/local/bin/chamber \
        ;; \
    "linux/arm64") \
        cp /usr/src/chamber/target/aarch64-unknown-linux-musl/release/chamber /usr/local/bin/chamber \
        ;; \
    esac
